...
ODIN_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "$ODIN_SCRIPT_DIR/.." && pwd)"

# shellcheck source=/dev/null
source "$ODIN_SCRIPT_DIR/lib/common.sh"
# shellcheck source=/dev/null
source "$ODIN_SCRIPT_DIR/lib/setup/steps.sh" # reuse allocate_port_base + helpers
...
local_ensure_env() {
  echo "📂 Preparing .env..."
  if [ ! -f .env ]; then
    if [ ! -f .odin/env.local.template ]; then
      error ".odin/env.local.template not found in $ROOT_DIR"
      return 1
    fi
...
local_allocate_ports() {
  ...
  LOCAL_DB_PROJECT="odin-$(sanitize_name "${ODIN_WORKSPACE_NAME:-$(basename "$PWD")}")"
...
local_write_env() {
  ...
  if [ -z "${ODIN_PORT_BASE:-}" ] || [ -z "$LOCAL_NEON_PROXY_PORT" ]; then
...

  {
    ...
    write_env_var "ODIN_WORKSPACE_NAME" "${ODIN_WORKSPACE_NAME:-$(basename "$PWD")}"
...
DEVVARS
  ...
  cat > "$ODIN_SCRIPT_DIR/ports.json" <<PORTSJSON
{
  "ports": [
...
local_write_config_overlay() {
  echo "🔧 Writing .odin/config.local.json (untracked overlay)..."
  cat > "$ODIN_SCRIPT_DIR/config.local.json" <<'CONFIGLOCAL'
{
  "setup": ["./.odin/setup.local.sh"],
...
local_setup_main() {
  ...
  echo "🚀 Setting up Odin for LOCAL development..."
  echo ""
