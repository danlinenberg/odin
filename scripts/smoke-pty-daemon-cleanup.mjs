...
async function main() {
	...
	const tmpDir = mkdtempSync(path.join(os.tmpdir(), "odin-pty-cleanup-"));
		...
		if (args.production || args.socketPath) {
			...
			daemonProcess = spawn(
				...
				["--experimental-strip-types", daemonEntry, `--socket=${socketPath}`],
				{
					...
					env: {
						...process.env,
						ODIN_PTY_DAEMON_VERSION: "cleanup-smoke",
					},
				},
			);
...
function listProductionDaemonManifests() {
	const home = process.env.ODIN_HOME_DIR || path.join(os.homedir(), ".odin");
