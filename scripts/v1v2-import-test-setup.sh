...
#   - odin-dev-data/local.db          (v1 fixtures)
#   - odin-dev-data/host/<orgId>/host.db  (host.db fixtures)
...
ODIN_ORG=a1b2c3d4-e5f6-7890-abcd-ef1234567890

DEV_DATA="$(pwd)/odin-dev-data"
DEV_DATA_LOCAL_DB="$DEV_DATA/local.db"
SATYA_TEST_HOST_DB="$DEV_DATA/host/$SATYA_TEST_ORG/host.db"
ODIN_HOST_DB="$DEV_DATA/host/$ODIN_ORG/host.db"
...
psql "$PGURL" >/dev/null <<SQL
INSERT INTO public.v2_projects (id, organization_id, name, slug, repo_clone_url) VALUES
  ('33333333-aaaa-4aaa-8aaa-000000000001', '$ODIN_ORG', 'cal.com (calcom)',      'v1v2-test-cal-com-calcom',     'https://github.com/calcom/cal.com'),
  ('33333333-aaaa-4aaa-8aaa-000000000002', '$ODIN_ORG', 'cal.com (onlook fork)', 'v1v2-test-cal-com-onlook-dev', 'https://github.com/onlook-dev/cal.com'),
  ('33333333-aaaa-4aaa-8aaa-000000000003', '$ODIN_ORG', 'onlook (test fixture)', 'v1v2-test-onlook',             'https://github.com/onlook-dev/onlook'),
  ('$ONBOOK_FIXTURE_V2_ID',                '$ODIN_ORG', 'onbook (test fixture)', 'v1v2-test-onbook',             'https://github.com/onlook-dev/onbook')
ON CONFLICT (id) DO NOTHING;
SQL
...
INSERT INTO worktrees (id, project_id, path, branch, base_branch, created_at, created_by_odin)
VALUES
...
echo "→ inserting host.db relocate row (Odin org)"
sqlite3 "$ODIN_HOST_DB" <<SQL
