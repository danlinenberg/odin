...
| File | Change |
|---|---|
| `apps/mobile/package.json` | deps on `@odin/host-service` (type-only) and `@trpc/server` (for `inferRouterOutputs`) |
| `apps/mobile/lib/host-service/client.ts` | `import type { AppRouter } from "@odin/host-service/router"`; output aliases via `inferRouterOutputs` |
...
1. Point `apps/mobile/lib/host-service/client.ts` at the contract package's types;
   drop mobile's `@odin/host-service` and `@trpc/server` deps.
