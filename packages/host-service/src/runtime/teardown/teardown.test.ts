...
describe("teardown initial command", () => {
	test("uses exec instead of shell-specific exit status syntax", () => {
		const command = buildTeardownInitialCommand(
			"/tmp/worktree/.odin/teardown.sh",
		);

		expect(command).toBe("exec bash '/tmp/worktree/.odin/teardown.sh'");
...
describe("resolveTeardownCommand", () => {
	function makeSandbox(): {
		...
		mkdirSync(join(repoPath, ".odin"), { recursive: true });
	...
	function writeConfig(repoPath: string, config: unknown): void {
		writeFileSync(
			join(repoPath, ".odin", "config.json"),
	...
	test("runs configured teardown commands from .odin/config.json", () => {
	...
	test("configured teardown takes precedence over a teardown.sh script", () => {
		...
		try {
			...
			writeFileSync(
				join(sb.repoPath, ".odin", "teardown.sh"),
				"#!/usr/bin/env bash\n",
			);
	...
	test("falls back to <repoPath>/.odin/teardown.sh when no teardown is configured", () => {
		...
		try {
			...
			const scriptPath = join(sb.repoPath, ".odin", "teardown.sh");
			writeFileSync(scriptPath, "#!/usr/bin/env bash\n");
