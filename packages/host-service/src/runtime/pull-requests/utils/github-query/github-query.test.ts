...
describe("GitHub pull request REST queries", () => {
	test("fetches only the PR matching the requested upstream head", async () => {
			...
			[
				{
					number: 42,
					title: "Fix sidebar",
					html_url: "https://github.com/danlinenberg/odin/pull/42",
					state: "open",
					draft: false,
					merged_at: null,
					...
					head: {
						...
						repo: {
							name: "odin",
							owner: { login: "danlinenberg" },
						},
					},
					base: {
						repo: {
							full_name: "danlinenberg/odin",
						},
					},
				},
			],
		...
		const result = await fetchPullRequestByHeadFromGh(
			execGh,
			{ owner: "danlinenberg", name: "odin" },
			{ owner: "danlinenberg", repo: "odin", branch: "fix/sidebar" },
		);

		expect(result).toEqual({
			number: 42,
			title: "Fix sidebar",
			url: "https://github.com/danlinenberg/odin/pull/42",
			state: "OPEN",
			isDraft: false,
			headRefName: "fix/sidebar",
			headRefOid: "abc123",
			isCrossRepository: false,
			headRepositoryOwner: { login: "danlinenberg" },
			headRepository: { name: "odin" },
		...
		expect(calls).toEqual([
			{
				args: [
					...
					"--method",
					"GET",
					"repos/danlinenberg/odin/pulls",
					"-f",
					"state=all",
					"-f",
					"head=danlinenberg:fix/sidebar",
					"-f",
					"sort=updated",
					"-f",
					"direction=desc",
			...
			[
				{
					number: 41,
					title: "Wrong fork",
					html_url: "https://github.com/danlinenberg/odin/pull/41",
					state: "open",
					draft: false,
					merged_at: null,
	...
	test("does not match a case-variant branch in per-head filtering", async () => {
			...
			[
				{
					number: 43,
					title: "Case drift",
					html_url: "https://github.com/danlinenberg/odin/pull/43",
					state: "open",
					draft: false,
					merged_at: null,
					updated_at: "2026-05-08T12:00:00Z",
		...
		const result = await fetchPullRequestByHeadFromGh(
			execGh,
			{ owner: "danlinenberg", name: "odin" },
			{ owner: "danlinenberg", repo: "odin", branch: "roshvan/fix-thing" },
		);
			...
			[
				{
					number: 44,
					title: "Open sweep",
					html_url: "https://github.com/danlinenberg/odin/pull/44",
					state: "open",
					draft: false,
					merged_at: null,
					updated_at: "2026-05-08T12:00:00Z",
		...
		expect(calls).toEqual([
			{
				args: [
					...
					"--method",
					"GET",
					"repos/danlinenberg/odin/pulls/42/reviews",
					"-f",
					"per_page=100",
				],
			},
				...
				check_runs: [
					{
						name: "Typecheck",
						conclusion: "success",
						details_url: "https://github.com/danlinenberg/odin/actions/1",
						status: "completed",
		...
		expect(result).toEqual([
			{
				...
				name: "Typecheck",
				conclusion: "SUCCESS",
				detailsUrl: "https://github.com/danlinenberg/odin/actions/1",
				status: "COMPLETED",
		...
		expect(calls).toEqual([
			{
				args: [
					...
					"--method",
					"GET",
					"repos/danlinenberg/odin/commits/abc123/check-runs",
					"-f",
					"per_page=100",
				],
			},
			{
				args: [
					...
					"--method",
					"GET",
					"repos/danlinenberg/odin/commits/abc123/statuses",
					"-f",
					"per_page=100",
				],
			},
