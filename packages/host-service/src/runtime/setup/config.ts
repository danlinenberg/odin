...
const PROJECT_ODIN_DIR_NAME = ".odin";
const CONFIG_FILE_NAME = "config.json";
const LOCAL_CONFIG_FILE_NAME = "config.local.json";
const ODIN_DIR_NAME = ".odin";
...
export function getProjectConfigPath(repoPath: string): string {
	return join(repoPath, PROJECT_ODIN_DIR_NAME, CONFIG_FILE_NAME);
}
...
function getUserOverridePaths(args: {
	...
	const projectsDir = join(args.homeDir, ODIN_DIR_NAME, PROJECTS_DIR_NAME);
...
function getLocalOverlayPath(repoPath: string): string {
	return join(repoPath, PROJECT_ODIN_DIR_NAME, LOCAL_CONFIG_FILE_NAME);
}
...
export function resolveScript(
	...
	for (const root of roots) {
		const scriptPath = join(root, PROJECT_ODIN_DIR_NAME, `${key}.sh`);
