...
function writeUserOverride(
	...
	const dir = join(homeDir, ".odin", "projects", projectId);
	mkdirSync(dir, { recursive: true });
...
function writeUserOverrideByPath(
	...
	const dir = join(homeDir, ".odin", "projects", repoPath);
	mkdirSync(dir, { recursive: true });
	writeFileSync(join(dir, "config.json"), JSON.stringify(content), "utf-8");
}
...
describe("resolveScript", () => {
	...
	function writeFallbackScript(key: string) {
		const dir = join(sandbox.repoPath, ".odin");
		mkdirSync(dir, { recursive: true });
	...
	it("falls back to <repoPath>/.odin/<key>.sh per key", () => {
	...
	it("prefers the worktree script when worktreePath is in scope", () => {
		...
		const worktreePath = join(sandbox.repoPath, ".worktrees", "feature");
		mkdirSync(join(worktreePath, ".odin"), { recursive: true });
		const worktreeScript = join(worktreePath, ".odin", "teardown.sh");
...
describe("getProjectConfigPath", () => {
	it("appends .odin/config.json to the repoPath", () => {
		expect(getProjectConfigPath("/tmp/x")).toBe("/tmp/x/.odin/config.json");
	});
});
