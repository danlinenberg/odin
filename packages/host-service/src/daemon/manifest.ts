...
function odinHomeDir(): string {
	return process.env.ODIN_HOME_DIR || join(homedir(), ".odin");
}

export function ptyDaemonManifestDir(organizationId: string): string {
	return join(odinHomeDir(), "host", organizationId);
}
...
export function listPtyDaemonManifests(): PtyDaemonManifest[] {
	const hostDir = join(odinHomeDir(), "host");
