...
import { ChatService } from "@odin/chat-legacy/server/desktop";
...
export function createApp(options: CreateAppOptions): CreateAppResult {
	...
	const acpSessionsEnabled =
		options.acpSessions !== undefined || process.env.ODIN_ACP_SESSIONS === "1";
	const acpSessions =
		options.acpSessions ??
		...
		trpcServer({
			...
			createContext: async (_opts, c) => {
				...
				return {
					...
					isAuthenticated,
					clientMachineId:
						c.req.header("x-odin-client-machine-id") ?? undefined,
