...
import { runWithPostCheckoutHookTolerance } from "@odin/shared/git-hook-tolerance";
import { generateFriendlyBranchName } from "@odin/shared/workspace-launch";
...
export const workspacesRouter = router({
	create: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			let workspaceRow: CloudShapedWorkspace;
			...
			if (input.pr !== undefined) {
				...
				try {
					...
					if (existing) {
						...
						if (localOid !== null && !adoptLocalBranch) {
							...
							throw new TRPCError({
								code: "CONFLICT",
								message: `Local branch "${resolvedBranch}" exists outside Odin and points at a different commit than PR #${input.pr} (local ${localOid.slice(0, 7)}, PR ${prMetadata.headRefOid.slice(0, 7)}). ${cleanupHint}`,
							});
						}
				...
				if (existing) {
					...
					if (existingWorktreePath) {
						...
						let adoptedRow: CloudShapedWorkspace | undefined;
						try {
