...
const DELIMITER = "ODIN_PROMPT_test1234";
...
describe("isChatAgent", () => {
	it("is true only for the odin chat agent", () => {
		expect(isChatAgent("odin")).toBe(true);
...
describe("validateAgentEffortSelection", () => {
	...
	it("rejects overrides for agents without effort support", () => {
		try {
			validateAgentEffortSelection("odin", "Odin", "high");
			throw new Error("Expected validation to fail");
		} catch (error) {
			expect(error).toBeInstanceOf(TRPCError);
			expect((error as TRPCError).code).toBe("BAD_REQUEST");
			expect((error as Error).message).toBe(
				"Odin does not support a reasoning effort override. Omit effort to use the agent default.",
			);
		}
	});
});
