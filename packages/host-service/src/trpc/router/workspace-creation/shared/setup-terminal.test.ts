...
describe("resolveInitialCommand", () => {
	...
	it("falls back to bash <repoPath>/.odin/setup.sh when config is empty", () => {
		writeConfig(sandbox.repoPath, { setup: [], teardown: [] });
		writeFallbackScript(sandbox.repoPath);

		expect(resolve()).toEqual({
			initialCommand: `bash '${join(sandbox.repoPath, ".odin", "setup.sh")}'`,
		});
	});
	...
	it("escapes single quotes in fallback path", () => {
		...
		try {
			...
			expect(cmd).toBe(
				`bash '${trickyRepo.replace("'", "'\\''")}/.odin/setup.sh'`,
			);
		} finally {
	...
	it("falls back to the worktree setup.sh before the main repo's", () => {
		...
		expect(
			...
			initialCommand: `bash '${join(worktreePath, ".odin", "setup.sh")}'`,
		});
	});
});
