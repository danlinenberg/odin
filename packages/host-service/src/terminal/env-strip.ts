...
const ODIN_KEEP_KEYS = new Set([
	"ODIN_HOME_DIR",
	"ODIN_AGENT_HOOK_PORT",
	"ODIN_AGENT_HOOK_VERSION",
]);
 ...
 * host-service but stop here. ODIN_REFRESH_TOKEN would already be caught
 * by the ODIN_ prefix rule, but listing it explicitly keeps the
 * protection load-bearing if ODIN_KEEP_KEYS ever changes.
 */
...
export function stripTerminalRuntimeEnv(
	...
	for (const [key, value] of Object.entries(baseEnv)) {
		...
		if (key.startsWith("ODIN_") && !ODIN_KEEP_KEYS.has(key)) continue;
