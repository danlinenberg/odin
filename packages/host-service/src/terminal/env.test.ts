...
describe("stripTerminalRuntimeEnv", () => {
	const secretsEnv: Record<string, string> = {
		...
		ODIN_AUTH_CONFIG_PATH: "/Users/test/.odin/config.json",
		...
		ODIN_API_URL: "https://api.example.com",
		...
		TURBO_TEAM: "my-team",
		// Legacy ODIN_* vars that should be stripped
		ODIN_PANE_ID: "pane-1",
		ODIN_TAB_ID: "tab-1",
		ODIN_PORT: "51741",
		ODIN_HOOK_VERSION: "2",
		ODIN_WORKSPACE_NAME: "my-ws",
		// Auth refresh tokens inherited from parent (CLI/desktop) env
		OAUTH_REFRESH_TOKEN: "oauth-refresh-secret",
		ODIN_REFRESH_TOKEN: "odin-refresh-secret",
		// Keys that SHOULD survive
		HOME: "/Users/test",
		PATH: "/usr/bin:/usr/local/bin",
		SHELL: "/bin/zsh",
		EDITOR: "vim",
		ODIN_HOME_DIR: "/Users/test/.odin",
		ODIN_AGENT_HOOK_PORT: "51741",
		ODIN_AGENT_HOOK_VERSION: "2",
	};
	...
	test("app/runtime secrets do not reach PTY env", () => {
		...
		expect(result.ODIN_AUTH_CONFIG_PATH).toBeUndefined();
		...
		expect(result.ODIN_API_URL).toBeUndefined();
	...
	test("refresh tokens do not reach PTY env", () => {
		...
		expect(result.ODIN_REFRESH_TOKEN).toBeUndefined();
	});
	...
	test("removed legacy vars do not reach PTY env", () => {
		const result = stripTerminalRuntimeEnv(secretsEnv);
		expect(result.ODIN_PANE_ID).toBeUndefined();
		expect(result.ODIN_TAB_ID).toBeUndefined();
		expect(result.ODIN_PORT).toBeUndefined();
		expect(result.ODIN_HOOK_VERSION).toBeUndefined();
		expect(result.ODIN_WORKSPACE_NAME).toBeUndefined();
	});
	...
	test("explicit Odin support keys are kept", () => {
		const result = stripTerminalRuntimeEnv(secretsEnv);
		expect(result.ODIN_HOME_DIR).toBe("/Users/test/.odin");
		expect(result.ODIN_AGENT_HOOK_PORT).toBe("51741");
		expect(result.ODIN_AGENT_HOOK_VERSION).toBe("2");
	});
...
describe("getShellLaunchArgs", () => {
	const odinHomeDir = "/tmp/test-odin";

	test("zsh launches as login shell", () => {
		expect(getShellLaunchArgs({ shell: "/bin/zsh", odinHomeDir })).toEqual([
			"-l",
		]);
	});

	test("bash falls back to login shell when rcfile missing", () => {
		const args = getShellLaunchArgs({ shell: "/bin/bash", odinHomeDir });
	...
	test("fish uses init-command", () => {
		...
		expect(args[2]).toContain("_odin_bin");
		expect(args[2]).toContain("133;A");
	});

	test("sh launches as login shell", () => {
		expect(getShellLaunchArgs({ shell: "/bin/sh", odinHomeDir })).toEqual([
			"-l",
		]);
	});

	test("ksh launches as login shell", () => {
		expect(getShellLaunchArgs({ shell: "/usr/bin/ksh", odinHomeDir })).toEqual([
			"-l",
		]);
	});

	test("unsupported shells launch natively without bootstrap", () => {
		expect(getShellLaunchArgs({ shell: "/usr/bin/pwsh", odinHomeDir })).toEqual(
			[],
		);
	});
});
...
describe("shellLaunchExpectsReadyMarker", () => {
	test("recognizes current zsh and bash wrappers", () => {
		const odinHomeDir = mkdtempSync(path.join(tmpdir(), "odin-shell-ready-"));
		try {
			mkdirSync(path.join(odinHomeDir, "zsh"), { recursive: true });
			mkdirSync(path.join(odinHomeDir, "bash"), { recursive: true });
			writeFileSync(path.join(odinHomeDir, "zsh", ".zshrc"), "# rc\n");
			writeFileSync(
				path.join(odinHomeDir, "zsh", ".zlogin"),
				'printf "\\033]133;A\\007"\n',
			);
			writeFileSync(
				path.join(odinHomeDir, "bash", "rcfile"),
				'printf "\\033]133;A\\007"\n',
			);
			...
			rmSync(odinHomeDir, { recursive: true, force: true });
		}
	});

	test("does not trust stale or incomplete wrapper files", () => {
		const odinHomeDir = mkdtempSync(path.join(tmpdir(), "odin-shell-stale-"));
		try {
			mkdirSync(path.join(odinHomeDir, "zsh"), { recursive: true });
			mkdirSync(path.join(odinHomeDir, "bash"), { recursive: true });
			writeFileSync(path.join(odinHomeDir, "zsh", ".zshrc"), "# rc\n");
	...
	test("recognizes fish's injected marker without wrapper files", () => {
		expect(
			shellLaunchExpectsReadyMarker({
				shell: "/usr/bin/fish",
				odinHomeDir: "/tmp/missing-odin-home",
			}),
		).toBe(true);
	});
});
...
describe("getShellBootstrapEnv", () => {
	test("zsh bootstrap applies only when wrapper files exist", () => {
		const result = getShellBootstrapEnv({
			...
			odinHomeDir: "/tmp/nonexistent-odin-dir",
		});
...
describe("buildV2TerminalEnv", () => {
	const baseParams = {
		...
		shell: "/bin/zsh",
		odinHomeDir: "/Users/test/.odin",
		cwd: "/tmp/workspace",
		terminalId: "term-1",
		workspaceId: "ws-1",
		workspacePath: "/tmp/workspace",
		rootPath: "/tmp/repo",
		odinEnv: "production" as const,
		agentHookPort: "51741",
		agentHookVersion: "2",
	};
	...
	test("injects the public terminal contract and retained v2 metadata", () => {
		...
		expect(env).toMatchObject({
			...
			COLORTERM: "truecolor",
			PWD: "/tmp/workspace",
			ODIN_TERMINAL_ID: "term-1",
			ODIN_WORKSPACE_ID: "ws-1",
			ODIN_WORKSPACE_PATH: "/tmp/workspace",
			ODIN_ROOT_PATH: "/tmp/repo",
			ODIN_ENV: "production",
			ODIN_AGENT_HOOK_PORT: "51741",
			ODIN_AGENT_HOOK_VERSION: "2",
		});
	...
	test("allows empty root path and alternate Odin env without breaking the contract", () => {
		const env = buildV2TerminalEnv({ ...baseParams, rootPath: "" });
		expect(env.ODIN_ROOT_PATH).toBe("");
		...
		expect(devEnv.ODIN_ENV).toBe("development");
		expect(devEnv.ODIN_ROOT_PATH).toBe("");
	});
	...
	test("drops removed v1 metadata while preserving user shell vars", () => {
		const env = buildV2TerminalEnv({
			...
			baseEnv: {
				...
				ODIN_PORT: "51741",
				ODIN_HOOK_VERSION: "2",
				ODIN_WORKSPACE_NAME: "my-workspace",
				NVM_DIR: "/Users/test/.nvm",
				SSH_AUTH_SOCK: "/tmp/ssh.sock",
			},
		});
		expect(env.ODIN_PANE_ID).toBeUndefined();
		expect(env.ODIN_TAB_ID).toBeUndefined();
		expect(env.ODIN_PORT).toBeUndefined();
		expect(env.ODIN_HOOK_VERSION).toBeUndefined();
		expect(env.ODIN_WORKSPACE_NAME).toBeUndefined();
