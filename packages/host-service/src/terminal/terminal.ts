...
import { hasRunningForegroundProcess } from "@odin/pty-daemon/process-tree";
import {
	createScanState,
...
} from "@odin/shared/terminal-title-scanner";
...
// Scanner logic lives in @odin/shared/shell-ready-scanner.
// ---------------------------------------------------------------------------
...
function reachableDaemonSocketPath(): string | null {
	const explicitSocket = process.env.ODIN_PTY_DAEMON_SOCKET;
...
export async function createTerminalSessionInternal({
	...
	const shellArgs = getShellLaunchArgs({ shell, odinHomeDir });
	const ptyEnv = buildV2TerminalEnv({
		...
		agentHookPort: process.env.ODIN_AGENT_HOOK_PORT || "",
		agentHookVersion: process.env.ODIN_AGENT_HOOK_VERSION || "",
