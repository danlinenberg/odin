...
export function getOdinShellPaths(odinHomeDir: string): {
	BIN_DIR: string;
	ZSH_DIR: string;
	BASH_DIR: string;
} {
	return {
		BIN_DIR: path.join(odinHomeDir, "bin"),
		ZSH_DIR: path.join(odinHomeDir, "zsh"),
		BASH_DIR: path.join(odinHomeDir, "bash"),
	};
}
...
export function getShellBootstrapEnv(
	...
	const { shell, baseEnv, odinHomeDir } = params;
	const shellName = getShellName(shell);
	const paths = getOdinShellPaths(odinHomeDir);

	if (shellName === "zsh") {
		const zshrc = path.join(paths.ZSH_DIR, ".zshrc");
		if (existsSync(zshrc)) {
			return {
				ODIN_ORIG_ZDOTDIR: baseEnv.ZDOTDIR || baseEnv.HOME || homedir(),
...
/**
 * Whether this exact launch configuration installs Odin's prompt marker.
 *
...
export function shellLaunchExpectsReadyMarker(
	...
	const { shell, odinHomeDir } = params;
	const shellName = getShellName(shell);
	const paths = getOdinShellPaths(odinHomeDir);
