...
Host-service owns the lifecycle of `@odin/pty-daemon` — the long-lived
...
- **Singleton + bootstrap**: `src/daemon/singleton.ts` — process-level
  ...
  pattern below.
- **Manifest**: `src/daemon/manifest.ts` — `$ODIN_HOME_DIR/host/{orgId}/pty-daemon-manifest.json`.
...
The socket path lives in `os.tmpdir()/odin-ptyd-<sha256(orgId).slice(0,12)>.sock`
...
(`$ODIN_HOME_DIR/host/{orgId}/{host-service,pty-daemon}.log`)
...
Setting `ODIN_PTY_DAEMON_SOCKET` env var bypasses the supervisor in
