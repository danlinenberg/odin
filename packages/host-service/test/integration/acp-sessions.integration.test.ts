...
`,
		...
		execSync(
			"git init -q && git add -A && git -c user.email=m2@odin.local -c user.name=m2 commit -qm init",
			{ cwd: workspaceDir },
		);
	...
	test("parallel real tool calls serialize adapter permissions one at a time", async () => {
		...
		// once. Odin's manager supports true concurrency (covered by the fake
