...
describe("bug-hunt-v2: workspaceCleanup.destroy phase ordering", () => {
	...
	test("destroy rejects a main workspace BEFORE running teardown", async () => {
		...
		// phase-0 main-workspace guard fires first, so the destroy is
		// rejected even if teardown would otherwise be skipped. Real
		// TEARDOWN_FAILED behavior would need a PTY-enabled harness.
