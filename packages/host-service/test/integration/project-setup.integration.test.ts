...
describe("project.setup error paths", () => {
	...
	test("rejects clone when the project has no repoCloneUrl", async () => {
...
describe("project.setup with caller-supplied origin (cross-host)", () => {
	let host: TestHost;
	...
	test("import with origin applies the origin name", async () => {
		host = await createTestHost();
		repo = await createGitFixture();
		const projectId = randomUUID();
	...
	test("clone with an unparseable origin repoCloneUrl fails", async () => {
		host = await createTestHost();
...
describe("project.create empty mode", () => {
	let host: TestHost;
	...
	test("creates repo dir, named row, and main workspace", async () => {
		host = await createTestHost();
		...
		process.env.GIT_AUTHOR_EMAIL = "test@odin.local";
		process.env.GIT_COMMITTER_NAME = "Test Runner";
		process.env.GIT_COMMITTER_EMAIL = "test@odin.local";
		try {
