 ...
 * `builtin-terminal-agents.ts`) plus the virtual `"odin"` chat agent.
...
export interface OdinChatModel extends AgentModelOption {
	provider: string;
}
 ...
 */
export const ODIN_CHAT_MODELS: readonly OdinChatModel[] = [
...
export const AGENT_MODEL_SUPPORT: readonly AgentModelSupport[] = [
	...
	},
	{
		presetId: "odin",
		modelFlag: null,
		models: ODIN_CHAT_MODELS.map(({ id, label }) => ({ id, label })),
	},
];
