...
describe("AGENT_MODEL_SUPPORT", () => {
	it("only references builtin presets (or the odin chat agent)", () => {
		const validIds = new Set<string>([...BUILTIN_TERMINAL_AGENT_TYPES, "odin"]);
		for (const entry of AGENT_MODEL_SUPPORT) {
			expect(validIds.has(entry.presetId)).toBe(true);
		}
	});

	it("has a model flag, a model env, or (odin) neither", () => {
		for (const entry of AGENT_MODEL_SUPPORT) {
			if (entry.presetId === "odin") {
...
describe("buildAgentModelArgs", () => {
	...
	it("returns [] for odin (model travels via chat metadata)", () => {
		expect(buildAgentModelArgs("odin", "anthropic/claude-opus-4-8")).toEqual(
			[],
		);
	});
...
describe("getAgentEffortSupport", () => {
	...
	it("returns undefined for presets without effort support", () => {
		...
		expect(getAgentEffortSupport("odin")).toBeUndefined();
	});
});
