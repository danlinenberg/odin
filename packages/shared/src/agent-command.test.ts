...
describe("buildAgentPromptCommand", () => {
	it("adds `--` before codex prompt payload", () => {
		...
		expect(command).toContain(
			"codex --dangerously-bypass-approvals-and-sandbox --dangerously-bypass-hook-trust -- \"$(cat <<'ODIN_PROMPT_12345678'",
		);
	...
	it("does not change non-codex commands", () => {
		...
		expect(command).toStartWith(
			"claude --dangerously-skip-permissions \"$(cat <<'ODIN_PROMPT_abcdefgh'",
		);
	});
	...
	it("uses Amp interactive stdin mode for prompt launches", () => {
		...
		expect(command).toStartWith("amp <<'ODIN_PROMPT_amp1234'");
	...
	it("uses Amp interactive stdin mode for file launches", () => {
		const command = buildAgentFileCommand({
			filePath: ".odin/task-demo.md",
			agent: "amp",
		});

		expect(command).toBe("amp < '.odin/task-demo.md'");
	});
	...
	it("uses pi interactive mode for prompt launches", () => {
		...
		expect(command).toStartWith("pi \"$(cat <<'ODIN_PROMPT_pi1234'");
...
describe("kimi agent registration", () => {
	...
	it("runs prompt launches headlessly and resumes them in the TUI", () => {
		...
		expect(command).toStartWith("kimi -p \"$(cat <<'ODIN_PROMPT_kimi1234'");
...
describe("grok agent registration", () => {
	...
	it("seeds prompt launches into the interactive TUI positionally", () => {
		...
		expect(command).toStartWith(
			"grok --always-approve \"$(cat <<'ODIN_PROMPT_grok1234'",
		);
		expect(command).toEndWith('\n)"');
	});
