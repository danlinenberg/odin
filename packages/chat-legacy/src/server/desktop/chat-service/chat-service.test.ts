...
const originalOdinHomeDir = process.env.ODIN_HOME_DIR;
...
let testOdinHomeDir: string | null = null;
...
describe("ChatService OpenAI auth storage", () => {
	beforeEach(() => {
		...
		testOdinHomeDir = mkdtempSync(join(tmpdir(), "chat-service-test-"));
		process.env.ODIN_HOME_DIR = testOdinHomeDir;
	...
	afterEach(() => {
		if (testOdinHomeDir) {
			rmSync(testOdinHomeDir, { recursive: true, force: true });
			testOdinHomeDir = null;
		}
		if (originalOdinHomeDir) {
			process.env.ODIN_HOME_DIR = originalOdinHomeDir;
		} else {
