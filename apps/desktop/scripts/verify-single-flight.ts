 ...
 * the same org against an ISOLATED temp $ODIN_HOME_DIR. Only Electron and
...
async function runWorker(home: string, marker: string, label: string) {
	process.env.ODIN_HOME_DIR = home;
	...
	const realHostInfo = await import("@odin/shared/host-info");
	mock.module("@odin/shared/host-info", () => ({
		...realHostInfo,
...
async function main() {
	...
	console.log(`isolated ODIN_HOME_DIR=${home}`);
