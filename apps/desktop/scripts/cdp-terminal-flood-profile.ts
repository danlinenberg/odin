...
import { DEV_EMAIL, DEV_PASSWORD } from "@odin/shared/dev-credentials";
...
function parseArgs(argv: string[]): Options {
	const options: Options = {
		...
		homeDir: process.env.ODIN_HOME_DIR ?? resolve("odin-dev-data"),
		samples: 60,
		intervalMs: 150,
...
async function runPhase({
	...
	for (let index = 0; index < options.samples; index++) {
		...
		const marker = `__odin_${label}_${index}_${randomUUID().slice(0, 8)}__`;
