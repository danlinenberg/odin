...
- [ ] **A2. Prompt + text attachment** — drag `v2-launch-test-artifacts/trace.log`
      into the modal. After launch: verify `.odin/attachments/trace.log`
      exists in the worktree (terminal: `ls .odin/attachments/`).
      Claude prompt contains `![trace.log](.odin/attachments/trace.log)`.
...
## B. Happy-path — chat agent (Odin Chat)

Disable Claude (or set Odin Chat as preferred via order in settings).
