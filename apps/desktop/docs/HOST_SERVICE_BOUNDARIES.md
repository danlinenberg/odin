...
| Default `~/.odin/host.db` | `app.ts` | Injected by caller | `dbPath` is required, no default |
| `~/.odin/chat-anthropic-env.json` | `anthropic-runtime-env.ts` | Moves with `LocalModelProvider` | Desktop-only path |
...
import { createApp, PskHostAuthProvider, JwtApiAuthProvider } from "@odin/host-service";
import { LocalGitCredentialProvider } from "@odin/host-service/providers/desktop";
import { LocalModelProvider } from "@odin/host-service/providers/desktop";

createApp({
  config: {
    dbPath: path.join(orgDir, "host.db"),
    cloudApiUrl: env.ODIN_API_URL,
...
No `if (process.resourcesPath)`. No `if (platform() === "darwin")`. No `~/.odin` defaults. The host service is a pure server; the caller decides how it's configured.
