...
`@odin/panes` store, and pass any terminal startup command as transient
...
│         .writeFile → <worktree>/.odin/attachments/…     │
...
1. **Deep solve for binary transport.** Current fix for the
   ...
   Library-level change to `@odin/ui/ai-elements/prompt-input`
...
5. **Schema coupling between old and new IDB stores.** Dexie opened
   the hand-rolled store's existing DB (`odin-pending-attachments`,
