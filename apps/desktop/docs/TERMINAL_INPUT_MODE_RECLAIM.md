...
- `renderer/lib/terminal/terminalInputModeReclaimer.ts` — a thin xterm-parser
  ...
  (`?1004`), and the `OSC 777;odin-shell-ready` marker feed the core; the
  disarm is written back on a microtask.

**Marker choice:** reclaim keys on `OSC 777;odin-shell-ready` (emitted only by
Odin's shell wrappers), **not** the co-emitted FinalTerm `OSC 133;A` — `133;A`
