# External Files Written by Odin Desktop

This document lists all files written by the Odin desktop app outside of user projects.
...
- **Default**: `~/.odin/`
- **Named workspace**: `~/.odin-{workspace}/` (e.g. `~/.odin-my-feature/`)

This separation prevents multiple instances from interfering with each other.

## Files in `~/.odin[-{workspace}]/`

### `bin/` - Agent Wrapper Scripts

| File | Purpose |
|------|---------|
| `amp` | Wrapper for Amp CLI that preserves Odin terminal context |
...
| `droid` | Wrapper for Factory Droid CLI that preserves Odin hook integration |
...
agent commands and inject Odin-specific configuration.
...
| `opencode/plugin/odin-notify.js` | OpenCode plugin for lifecycle events |

## Global Tool Settings Files

Some CLIs only support global user settings for hook registration. Odin merges
...
For Codex specifically, Odin now relies on native `~/.codex/hooks.json`
registration for durable prompt/tool lifecycle events, while the wrapper in
`~/.odin[-{workspace}]/bin/codex` still injects `notify` and keeps the
session-log watcher as a best-effort compatibility bridge for older Codex
releases. On startup, Odin rewrites only its own managed entries in
...
- App-owned non-interactive `-c` command execution still routes managed binaries through absolute Odin wrapper paths.
...
Previously, the OpenCode plugin was written to `~/.config/opencode/plugin/odin-notify.js`.
...
The app modifies shell RC files to add the Odin bin directory to PATH:

| Shell | RC File | Modification |
|-------|---------|--------------|
| Zsh | `~/.zshrc` | Prepends `~/.odin[-{workspace}]/bin` to PATH |
| Bash | `~/.bashrc` | Prepends `~/.odin[-{workspace}]/bin` to PATH |
...
| Variable | Purpose |
|----------|---------|
| `ODIN_PANE_ID` | Unique identifier for the terminal pane |
| `ODIN_TAB_ID` | Identifier for the containing tab |
| `ODIN_WORKSPACE_ID` | Identifier for the workspace |
| `ODIN_WORKSPACE_NAME` | Human-readable workspace name |
| `ODIN_WORKSPACE_PATH` | Filesystem path to the workspace |
| `ODIN_ROOT_PATH` | Root path of the project |
| `ODIN_PORT` | Port for the notification server |
| `ODIN_ENV` | Environment (`development` or `production`) |
| `ODIN_HOOK_VERSION` | Hook protocol version for compatibility |

## Adding New External Files

Before adding new files outside of `~/.odin[-{workspace}]/`:
...
2. Verify `ODIN_ENV` and `ODIN_PORT` are set correctly in terminal
3. Delete stale global files: `rm -rf ~/.config/opencode/plugin/odin-notify.js`
