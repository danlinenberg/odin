...
const COPILOT_HOOK_SIGNATURE = "# Odin copilot hook";
...
# Auto-inject Odin notification hooks when running inside a v2 Odin terminal.
if [ -n "$ODIN_TERMINAL_ID" ] && [ -f "${hookScriptPath}" ]; then
  COPILOT_HOOKS_DIR=".github/hooks"
  COPILOT_HOOK_FILE="$COPILOT_HOOKS_DIR/odin-notify.json"
  ...
  if [ -d ".git/info" ]; then
    grep -qF ".github/hooks/odin-notify.json" ".git/info/exclude" 2>/dev/null || \\
      printf '%s\\n' ".github/hooks/odin-notify.json" >> ".git/info/exclude" 2>/dev/null
  fi
fi
