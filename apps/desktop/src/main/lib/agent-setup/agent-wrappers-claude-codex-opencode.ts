...
const OPENCODE_PLUGIN_SIGNATURE = "// Odin opencode plugin";
...
/**
 * Returns the environment-scoped OpenCode plugin path under Odin home.
 */
...
const CLAUDE_DYNAMIC_NOTIFY_PATH_MARKER = `$ODIN_HOME_DIR/${MANAGED_NOTIFY_RELATIVE_PATH}`;
...
/**
 * Writes Odin hook definitions directly into ~/.claude/settings.json.
 ...
 * (createClaudeSettingsJson); the wrapper exists only to forward ODIN_*
...
/**
 * Creates the Codex wrapper that injects Odin's notify/session-log logic.
 */
 ...
 *   { hooks: { EventName: [{ matcher?, hooks: [{ type, command }] }] } }
 *
 * Odin uses native Codex hooks as the durable lifecycle integration path.
...
export function getCodexGlobalHooksJsonContent(
	...
	// Remove all stale Odin-managed Codex hook commands, including events we
	...
	// Inline ODIN_AGENT_ID like getClaudeManagedHookCommand so the v2
	...
	const codexCommand = `ODIN_AGENT_ID=codex "${notifyScriptPath}"`;
...
/**
 * Writes Odin hook definitions directly into ~/.codex/hooks.json.
