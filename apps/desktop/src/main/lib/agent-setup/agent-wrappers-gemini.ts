...
const GEMINI_HOOK_SIGNATURE = "# Odin gemini hook";
	...
	for (const eventName of eventNames) {
		...
		const { entries } = reconcileManagedEntries({
			...
			isManaged: (definition: GeminiHookDefinition) =>
				isOdinManagedHookCommand(definition.command, GEMINI_HOOK_SCRIPT_NAME) ||
				Boolean(
					definition.hooks?.some(
						(hook) =>
							hook.command?.includes(hookScriptPath) ||
							isOdinManagedHookCommand(hook.command, GEMINI_HOOK_SCRIPT_NAME),
					),
				),
			isEquivalent: (
