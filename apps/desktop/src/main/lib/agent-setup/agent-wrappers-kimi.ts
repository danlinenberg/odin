...
export const KIMI_HOOKS_MARKER_START =
	"# >>> odin-managed-kimi-hooks v1 (do not edit) >>>";
export const KIMI_HOOKS_MARKER_END = "# <<< odin-managed-kimi-hooks v1 <<<";
...
function isManagedOrPartialHookTable(lines: string[]): boolean {
	...
	return !commandLine || commandLine.includes("ODIN_AGENT_ID=kimi");
}
...
/** Preserve user config while replacing Odin's marker-owned hook block. */
