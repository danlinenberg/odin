...
const TEST_ROOT = path.join(
	os.tmpdir(),
	`odin-managed-skills-${process.pid}-${Date.now()}`,
);
...
const claudePlugin = path.join(claudeSkills, "odin");
const agentsSkills = path.join(HOME_DIR, ".agents", "skills");
const commandsDir = path.join(HOME_DIR, ".agents", "commands", "odin");
...
function seedBundledPlugin(): void {
	...
	writeFileSync(
		...
		JSON.stringify({ name: "odin", version: "0.3.0" }),
	);
...
describe("setFrontmatterName", () => {
	it("rewrites only the frontmatter name", () => {
		const renamed = setFrontmatterName(skillMd("feedback"), "odin-feedback");
		expect(renamed).toContain("name: odin-feedback");
...
describe("createManagedSkills", () => {
	...
	it("provisions prefixed skill dirs for other agents with rewritten names", async () => {
		...
		for (const dirName of ["odin-feedback", "odin-10x", "odin-orchestrate"]) {
		...
		expect(
			existsSync(
				path.join(agentsSkills, "odin-orchestrate", "agents", "openai.yaml"),
			),
		).toBe(true);
	});
	...
	it("never touches a user-owned plugin dir or files without markers", async () => {
		...
		const userSkill = path.join(agentsSkills, "odin-feedback", "SKILL.md");
	...
	it("reaps stale managed dirs from earlier versions but keeps user dirs", async () => {
		const staleClaude = path.join(claudeSkills, "odin-feedback");
		mkdirSync(staleClaude, { recursive: true });
		writeFileSync(
			path.join(staleClaude, "SKILL.md"),
			withManagedMarker(skillMd("odin-feedback")),
		);
		const staleAgents = path.join(agentsSkills, "odin-orchestration");
		mkdirSync(staleAgents, { recursive: true });
		writeFileSync(
			path.join(staleAgents, "SKILL.md"),
			withManagedMarker(skillMd("odin-orchestration")),
		);
	...
	it("automatically ships any skill added to the bundled plugin", async () => {
		...
		expect(
			readFileSync(
				path.join(agentsSkills, "odin-newskill", "SKILL.md"),
				"utf-8",
			),
		).toContain("name: odin-newskill");
		expect(
