{{MARKER}}
/**
 * Odin Notification Plugin for OpenCode
 *
 ...
 * - Debug logging: Set ODIN_DEBUG=1 to enable verbose logging
 *
 * SUBAGENT FILTERING:
 ...
 */
export const OdinNotifyPlugin = async ({ $, client }) => {
  if (globalThis.__odinOpencodeNotifyPluginV8) return {};
  globalThis.__odinOpencodeNotifyPluginV8 = true;

  // Only run inside a v2 Odin terminal session.
  if (!process?.env?.ODIN_TERMINAL_ID) return {};

  const notifyPath = "{{NOTIFY_PATH}}";
  const debug = process?.env?.ODIN_DEBUG === '1';
  ...
  const log = (...args) => {
    if (debug) console.log('[odin-plugin]', ...args);
  };

  /**
   * Sends a notification to Odin's notification server.
  ...
  const notify = async (hookEventName) => {
    ...
    try {
      // ODIN_AGENT_ID=opencode is exported by the opencode wrapper and
