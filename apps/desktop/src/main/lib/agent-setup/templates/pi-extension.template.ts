{{MARKER}}
/**
 * Odin Notification Extension for pi
 *
 * Emits Claude-Code-compatible lifecycle hooks to Odin's notify.sh so
 * the host UI gets a "working" indicator (and completion chime) for pi
 * sessions, the same way it does for Claude Code, Codex, etc.
 *
 * Mapping:
 *   pi `before_agent_start`  → Claude `UserPromptSubmit`  → Odin `Start`
 ...
 *
 * Activates only when running inside a v2 Odin terminal (detected via
 * ODIN_TERMINAL_ID). Outside Odin it's a complete no-op. If notify.sh
 * is missing it's also a no-op (Odin uninstalled / never installed).
 *
...
export default function (pi: ExtensionAPI) {
	// Only activate inside a v2 Odin terminal.
	if (!process.env.ODIN_TERMINAL_ID) return;

	const odinHome =
		process.env.ODIN_HOME_DIR || join(homedir(), ".odin");
	const notifyScript = join(odinHome, "hooks", "notify.sh");
	...
	// helper and should NOT drive Odin's working indicator. Interactive
	...
	// Odin working indicator doesn't get stuck on. Fires on Ctrl+C,
