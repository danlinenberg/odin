...
export const GROK_COMPAT_MARKER_START =
	"# >>> odin-managed-grok-compat v1 (do not edit) >>>";
export const GROK_COMPAT_MARKER_END = "# <<< odin-managed-grok-compat v1 <<<";

export const GROK_HOOKS_FILE = "odin-notify.json";
...
// Vendor hook configs Odin also manages. Grok replays them in compat mode
// with their inlined ODIN_AGENT_ID (claude/cursor-agent), which would
...
export function getGrokConfigTomlContent(existing: string): string {
	...
	const block = [
		...
		"# Odin registers its own Grok hooks; replaying Odin-managed",
