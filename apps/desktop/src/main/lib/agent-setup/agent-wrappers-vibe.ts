...
export const VIBE_HOOKS_MARKER_START =
	"# >>> odin-managed-hooks v1 (do not edit) >>>";
export const VIBE_HOOKS_MARKER_END = "# <<< odin-managed-hooks v1 <<<";
...
function buildVibeManagedHooksBlock(): string {
	return [
		...
		"",
		"[[hooks]]",
		'name = "odin-notify-post-agent-turn"',
...
const MANAGED_HOOK_NAME_PREFIX = "odin-notify-";
 ...
 * block only ever contains `[[hooks]]` tables named `odin-notify-*`, so we
