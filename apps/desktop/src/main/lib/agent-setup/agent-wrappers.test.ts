...
const TEST_ROOT = path.join(
	realOs.tmpdir(),
	`odin-agent-wrappers-${process.pid}-${Date.now()}`,
);
const TEST_BIN_DIR = path.join(TEST_ROOT, "odin", "bin");
const TEST_HOOKS_DIR = path.join(TEST_ROOT, "odin", "hooks");
const TEST_ZSH_DIR = path.join(TEST_ROOT, "odin", "zsh");
const TEST_BASH_DIR = path.join(TEST_ROOT, "odin", "bash");
...
mock.module("./notify-hook", () => ({
	...
	NOTIFY_SCRIPT_MARKER: "# Odin agent notification hook v5",
...
describe("reconcileManagedEntries", () => {
	it("preserves user-managed entries while replacing stale managed entries", () => {
		const result = reconcileManagedEntries({
			current: [
				...
				"/tmp/.odin-old/hooks/notify.sh Start",
			],
			desired: ["/tmp/.odin-new/hooks/notify.sh Start"],
			isManaged: (entry: string) => entry.includes("/.odin-"),
			isEquivalent: (entry: string, desired: string) => entry === desired,
		});

		expect(result.entries).toEqual([
			"/usr/local/bin/custom-hook Start",
			"/tmp/.odin-new/hooks/notify.sh Start",
		]);
	...
	it("reconciles edited managed entries even when a managed hook already exists", () => {
		const result = reconcileManagedEntries({
			current: ["/tmp/.odin-current/hooks/notify.sh Start --debug"],
			desired: ["/tmp/.odin-current/hooks/notify.sh Start"],
			isManaged: (entry: string) => entry.includes("/.odin-"),
			isEquivalent: (entry: string, desired: string) => entry === desired,
		});

		expect(result.entries).toEqual([
			"/tmp/.odin-current/hooks/notify.sh Start",
		]);
		expect(result.replacedManagedEntries).toEqual([
			"/tmp/.odin-current/hooks/notify.sh Start --debug",
		]);
	});
});
...
describe("agent-wrappers copilot", () => {
	...
	it("rewrites stale odin-notify.json with current hook path", () => {
		const projectDir = path.join(TEST_ROOT, "project");
		const hooksDir = path.join(projectDir, ".github", "hooks");
		const hookFile = path.join(hooksDir, "odin-notify.json");
		...
		writeFileSync(hookFile, '{"odin":"old","bash":"/tmp/old-hook.sh"}');
	...
	it("tails codex's process-scoped TUI session log to drive Start events", () => {
		...
		expect(wrapper).toContain(
			`"$REAL_BIN" "\${_odin_codex_args[@]}" --enable hooks -c 'notify=["bash","${path.join(TEST_HOOKS_DIR, "notify.sh")}"]' "$@"`,
		);
		expect(wrapper).toContain('export ODIN_AGENT_ID="codex"');

		expect(wrapper).toContain("# Odin agent-wrapper v3");
		...
		expect(wrapper).toContain("ODIN_CODEX_SESSION_WATCHER_PID");
		expect(wrapper).toContain("CODEX_TUI_RECORD_SESSION");
		expect(wrapper).toContain("CODEX_TUI_SESSION_LOG_PATH");
		expect(wrapper).toContain("ODIN_TERMINAL_ID$ODIN_TAB_ID");
		expect(wrapper).toContain("_odin_configure_project_trust");
		expect(wrapper).toContain("ODIN_WORKSPACE_PATH/.codex");
		expect(wrapper).toContain(
			'projects={\\"$_odin_workspace_path_toml\\"={trust_level=\\"trusted\\"}}',
		);
		expect(wrapper).not.toContain("export CODEX_HOME=");
		expect(wrapper).not.toContain("rollout-*.jsonl");
		expect(wrapper).not.toContain("_odin_sessions_dir");
		expect(wrapper).not.toContain("$" + "{CODEX_HOME:-$HOME/.codex}");
		expect(wrapper).toContain("ODIN_HOOK_DEBUG_LOG");
		expect(wrapper).toContain("tail -n +1 -F");
		expect(wrapper).toContain("_odin_cleanup_session_watcher");
		expect(wrapper).toContain("_odin_child_pids_for");
		expect(wrapper).toContain('kill -TERM "$_odin_child_pid"');
		expect(wrapper).toContain('kill -KILL "$_odin_watcher_pid"');
		expect(wrapper).not.toContain("mkfifo");
		expect(wrapper).not.toContain("ODIN_CODEX_SESSION_WATCHER_TAIL_PID_PATH");
	...
	it("trusts the Odin workspace codex project config without replacing CODEX_HOME", () => {
		const realBinDir = path.join(TEST_ROOT, "real-bin");
...
`,
		...
		execFileSync(wrapperPath, [], {
			env: {
				...
				PATH: `${TEST_BIN_DIR}:${realBinDir}:${process.env.PATH || ""}`,
				ODIN_DEBUG_HOOKS: "1",
				ODIN_HOOK_DEBUG_LOG: debugLogPath,
				ODIN_TERMINAL_ID: "terminal-1",
			},
			encoding: "utf-8",
		});
...
`,
	...
	it("creates mastracode wrapper passthrough", () => {
		...
		expect(wrapper).toContain("# Odin wrapper for mastracode");
	...
	it("creates amp wrapper passthrough", () => {
		...
		expect(wrapper).toContain("# Odin wrapper for amp");
		expect(wrapper).toContain('REAL_BIN="$(find_real_binary "amp")"');
		expect(wrapper).toContain('export ODIN_AGENT_ID="amp"');
		expect(wrapper).toContain('exec "$REAL_BIN" "$@"');
	});
	...
	it("creates Amp lifecycle plugin", () => {
		...
		expect(plugin).toContain('ODIN_AGENT_ID: "amp"');
		expect(plugin).toContain("[odin-amp-plugin]");
		expect(plugin).toContain("ODIN_HOME_DIR");
	});
	...
	it("creates droid wrapper passthrough", () => {
		...
		expect(wrapper).toContain("# Odin wrapper for droid");
		expect(wrapper).toContain('REAL_BIN="$(find_real_binary "droid")"');
		expect(wrapper).toContain('export ODIN_AGENT_ID="droid"');
		expect(wrapper).toContain('exec "$REAL_BIN" "$@"');
	});

	it("replaces stale Cursor hook commands from old odin paths", () => {
		const cursorHooksPath = path.join(mockedHomeDir, ".cursor", "hooks.json");
		const staleHookPath = "/tmp/worktree/odin-dev-data/hooks/cursor-hook.sh";
		const currentHookPath = "/tmp/.odin-new/hooks/cursor-hook.sh";
	...
	it("replaces stale Gemini hook commands from old odin paths", () => {
		const geminiSettingsPath = path.join(
			mockedHomeDir,
			".gemini",
			"settings.json",
		);
		const staleHookPath = "/tmp/worktree/odin-dev-data/hooks/gemini-hook.sh";
		const currentHookPath = "/tmp/.odin-new/hooks/gemini-hook.sh";
	...
	it("bumps hook script markers when hook semantics change", () => {
		expect(COPILOT_HOOK_MARKER).toBe("# Odin copilot hook v2");
		expect(CURSOR_HOOK_MARKER).toBe("# Odin cursor hook v4");
		expect(GEMINI_HOOK_MARKER).toBe("# Odin gemini hook v3");
	});

	it("replaces stale Mastra hook commands from old odin paths", () => {
		const mastraHooksPath = path.join(
			mockedHomeDir,
			".mastracode",
			"hooks.json",
		);
		const staleHookPath = "/tmp/.odin-old/hooks/notify.sh";
		const currentHookPath = "/tmp/.odin-new/hooks/notify.sh";
			...
			expect(
				hooks.some(
					(entry) =>
						entry.type === "command" &&
						entry.command ===
							`ODIN_AGENT_ID=mastracode bash '${currentHookPath}'`,
				),
			).toBe(true);
	...
	it("replaces stale Droid hook commands from old odin paths", () => {
		...
		for (const eventName of managedEvents) {
			...
			expect(
				hooks.some((def) =>
					def.hooks.some(
						(hook) =>
							hook.command === `ODIN_AGENT_ID=droid '${currentHookPath}'`,
					),
				),
			).toBe(true);
			expect(
	...
	it("skips Droid settings writes when the existing JSON is invalid", () => {
		...
		expect(
			getDroidSettingsJsonContent("/tmp/.odin-new/hooks/notify.sh"),
		).toBeNull();
...
describe("agent-wrappers claude settings.json", () => {
	...
	it("replaces stale Claude hook commands from old odin paths", () => {
		const claudeSettingsPath = path.join(
			mockedHomeDir,
			".claude",
			"settings.json",
	...
	it("skips Claude settings writes when existing JSON is invalid", () => {
		...
		expect(
			getClaudeGlobalSettingsJsonContent("/tmp/.odin/hooks/notify.sh"),
		).toBeNull();
...
describe("agent-wrappers codex hooks.json", () => {
	...
	it("creates Codex hooks.json with prompt and lifecycle hooks when no file exists", () => {
		...
		const expectedCommand = `ODIN_AGENT_ID=codex "${notifyPath}"`;
		for (const eventName of [
			"SessionStart",
			"UserPromptSubmit",
			"Stop",
	...
	it("preserves user hooks when merging", () => {
		...
		const expectedManagedCommand = `ODIN_AGENT_ID=codex "${notifyPath}"`;
	...
	it("replaces stale Codex hook commands from old odin paths", () => {
		const codexHooksPath = path.join(mockedHomeDir, ".codex", "hooks.json");
		const staleHookPath = "/tmp/.odin-old/hooks/notify.sh";
		const currentHookPath = "/tmp/.odin-new/hooks/notify.sh";
		...
		const expectedManagedCommand = `ODIN_AGENT_ID=codex "${currentHookPath}"`;
		for (const eventName of [
			"SessionStart",
			"UserPromptSubmit",
			"Stop",
	...
	it("removes stale Odin-managed UserPromptSubmit hooks without touching user hooks", () => {
		const codexHooksPath = path.join(mockedHomeDir, ".codex", "hooks.json");
		const staleHookPath =
			"/Users/test/.odin/worktrees/repo/odin-dev-data/hooks/notify.sh";
		const currentHookPath = "/tmp/.odin-new/hooks/notify.sh";
	...
	it("reaps stale notify.sh paths from in-repo dev worktrees", () => {
		...
		// and its dev setup writes ODIN_HOME_DIR=<worktree>/odin-dev-data.
		// There is no /.odin/ segment anywhere in the path.
		const staleHookPath =
			"/Users/test/code/odin/.worktrees/old-branch/odin-dev-data/hooks/notify.sh";
		const currentHookPath = "/tmp/.odin-new/hooks/notify.sh";
	...
	it("skips Codex hooks writes when existing JSON is invalid", () => {
		...
		expect(
			getCodexGlobalHooksJsonContent("/tmp/.odin/hooks/notify.sh"),
		).toBeNull();
...
describe("vibe wrapper", () => {
	it("enables experimental hooks and stamps the agent id", () => {
		...
		expect(script).toContain('export ODIN_AGENT_ID="vibe"');
...
describe("vibe hooks.toml", () => {
	it("writes both managed hooks inside markers on an empty file", () => {
		...
		expect(out).toContain("ODIN_AGENT_ID=vibe");
	});
	...
	it("preserves user hooks that follow an orphaned start marker", () => {
		...
		expect(out.split(VIBE_HOOKS_MARKER_START).length - 1).toBe(1);
		expect(out.split(VIBE_HOOKS_MARKER_END).length - 1).toBe(1);
		expect(out.split('name = "odin-notify-before-tool"').length - 1).toBe(1);
	});
});
...
describe("kimi wrapper", () => {
	it("stamps the agent id and forwards arguments to the real binary", () => {
		...
		expect(script).toContain('export ODIN_AGENT_ID="kimi"');
		expect(script).toContain('exec "$REAL_BIN" "$@"');
	});
});
...
describe("kimi config.toml", () => {
	it("registers the lifecycle hooks Kimi exposes", () => {
		...
		expect(out).toContain("ODIN_AGENT_ID=kimi");
	});
	...
	it("preserves user hooks after an orphaned managed block", () => {
		const partial = [
			...
			"[[hooks]]",
			'event = "SessionStart"',
			"command = 'ODIN_AGENT_ID=kimi true'",
			"",
			"# user hook",
			"[[hooks]]",
			'event = "Notification"',
...
describe("grok wrapper", () => {
	it("stamps the agent id and forwards arguments to the real binary", () => {
		...
		expect(script).toContain('export ODIN_AGENT_ID="grok"');
		expect(script).toContain('exec "$REAL_BIN" "$@"');
	});
});
...
describe("grok hooks json", () => {
	it("registers passive lifecycle hooks and skips blocking PreToolUse", () => {
		...
		for (const definitions of Object.values(parsed.hooks)) {
			...
			expect(definition.hooks[0].command).toContain("ODIN_AGENT_ID=grok");
		}
...
describe("agent-wrappers pi", () => {
	...
	it("installs the pi extension into the global ~/.pi/agent/extensions directory", () => {
		...
		expect(extensionPath).toBe(
			path.join(mockedHomeDir, ".pi", "agent", "extensions", "odin-hooks.ts"),
		);
