...
export interface QuitCleanupDeps {
	...
	disposeTray: () => void;
	stopNetworkLogger: () => Promise<void>;
...
export async function runQuitCleanup(deps: QuitCleanupDeps): Promise<void> {
	...
	try {
		...
		} else if (isDev || isUpdateInstalling) {
			// A dev quit is almost always an electron-vite hot-reload restart, and
			// terminal-host owns the PTY subprocesses: tearing it down killed every
			// session on every restart. Detach instead and let the next boot adopt
			// the daemon; a genuinely stale one is restarted by the bundle-hash check.
			disposeTerminalHostClient();
		}
		shutdownPersistence();
		disposeTray();
