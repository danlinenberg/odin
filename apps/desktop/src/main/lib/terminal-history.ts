...
export function getTerminalHistoryRootDir(): string {
	return join(homedir(), ODIN_DIR_NAME, TERMINAL_HISTORY_DIR_NAME);
}
...
export class HistoryReader {
	 ...
	 */
	async readScrollbackTail(
		bytes: number,
	): Promise<{ text: string; size: number } | null> {
		try {
			const { size } = await fs.stat(this.scrollbackPath);
			const start = Math.max(0, size - bytes);
			const handle = await fs.open(this.scrollbackPath, "r");
			try {
