...
describe("DaemonTerminalManager kill tracking", () => {
	...
	it("forwards no dimensions when the caller asked for none", async () => {
		...
		const attach = manager.createOrAttach({
			paneId,
		...
		expect(mockClient.createOrAttachCalls[0]?.cols).toBeUndefined();
		expect(mockClient.createOrAttachCalls[0]?.rows).toBeUndefined();

		mockClient.resolveCreateOrAttach("req-screen", 789);
		await attach;
	});

	it("forwards the dimensions a mounted view measured", async () => {
		const manager = new DaemonTerminalManager();
		const paneId = "pane-mounted-view";
		const managerInternals = manager as unknown as {
			daemonSessionIdsHydrated: boolean;
			daemonAliveSessionIds: Set<string>;
		};
		...
		expect(mockClient.createOrAttachCalls[0]?.cols).toBe(161);
		expect(mockClient.createOrAttachCalls[0]?.rows).toBe(42);

		mockClient.resolveCreateOrAttach("req-mounted-dims", 790);
		await attach;
	});
