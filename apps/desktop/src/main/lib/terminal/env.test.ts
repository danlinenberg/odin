...
describe("env", () => {
	...
	describe("buildSafeEnv", () => {
		...
		describe("includes allowlisted shell environment vars", () => {
			...
			it("should include shell wrapper control vars", () => {
				const env = {
					ZDOTDIR: "/Users/test/.odin-dev/zsh",
					BASH_ENV: "/Users/test/.odin-dev/bash/rcfile",
					PATH: "/usr/bin",
				};
				const result = buildSafeEnv(env);
				expect(result.ZDOTDIR).toBe("/Users/test/.odin-dev/zsh");
				expect(result.BASH_ENV).toBe("/Users/test/.odin-dev/bash/rcfile");
			});
		...
		describe("includes ODIN_* prefix vars", () => {
			it("should include ODIN_* vars (our metadata)", () => {
				const env = {
					ODIN_PANE_ID: "pane-1",
					ODIN_TAB_ID: "tab-1",
				...
				const result = buildSafeEnv(env);
				expect(result.ODIN_PANE_ID).toBe("pane-1");
				expect(result.ODIN_TAB_ID).toBe("tab-1");
				expect(result.ODIN_WORKSPACE_ID).toBe("ws-1");
			});
		});
		...
		describe("Windows platform case-insensitivity", () => {
			...
			it("should include Odin_* prefix vars case-insensitively on Windows", () => {
				const env = {
					Odin_Pane_Id: "pane-1",
					ODIN_TAB_ID: "tab-1",
					PATH: "/usr/bin",
	...
	describe("buildTerminalEnv", () => {
		...
		describe("terminal metadata", () => {
			...
			it("should set Odin-specific env vars", () => {
				const result = buildTerminalEnv(baseParams);
			...
			it("should handle optional workspace params", () => {
				...
				expect(result.ODIN_WORKSPACE_NAME).toBe("my-workspace");
				expect(result.ODIN_WORKSPACE_PATH).toBe("/path/to/workspace");
				expect(result.ODIN_ROOT_PATH).toBe("/root/path");
			});

			it("should default optional params to empty string", () => {
				const result = buildTerminalEnv(baseParams);

				expect(result.ODIN_WORKSPACE_NAME).toBe("");
				expect(result.ODIN_WORKSPACE_PATH).toBe("");
				expect(result.ODIN_ROOT_PATH).toBe("");
			});
			...
			it("should include ODIN_PORT", () => {
				const result = buildTerminalEnv(baseParams);
				expect(result.ODIN_PORT).toBeDefined();
				expect(typeof result.ODIN_PORT).toBe("string");
			});

			it("should preserve ODIN_HOME_DIR for app-launched hooks", () => {
				process.env.ODIN_HOME_DIR = "/tmp/odin-home";
				const result = buildTerminalEnv(baseParams);
				expect(result.ODIN_HOME_DIR).toBe("/tmp/odin-home");
			});
		});

		it("should include ODIN_ENV for dev/prod separation", () => {
			const result = buildTerminalEnv(baseParams);
			expect(result.ODIN_ENV).toBeDefined();
			expect(["development", "production"]).toContain(result.ODIN_ENV);
		});

		it("should include ODIN_HOOK_VERSION for protocol versioning", () => {
			const result = buildTerminalEnv(baseParams);
			expect(result.ODIN_HOOK_VERSION).toBeDefined();
			expect(result.ODIN_HOOK_VERSION).toBe("2");
		});
