...
export interface CreateOrAttachRequest {
	...
	requestId?: string;
	/**
	 * Viewport size to give the PTY. Omit when attaching only to read the
	 * screen: a caller with no view of its own must not resize a live session
	 * out from under the one that does.
	 */
	cols?: number;
	rows?: number;
	cwd?: string;
