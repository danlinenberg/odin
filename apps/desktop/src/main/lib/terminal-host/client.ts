...
} from "@odin/pty-daemon/process-tree";
...
const DEBUG_CLIENT = process.env.ODIN_TERMINAL_DEBUG === "1";
...
export class TerminalHostClient extends EventEmitter {
	...
	/**
	 * Hash the daemon bundle. Content, not mtime: electron-vite rewrites every
	 * main-process entry on any main-process edit, so an mtime comparison called
	 * the daemon stale on every hot-reload restart and killed every session with
	 * it. Identical bytes mean the running daemon is still the right code.
	 */
	private hashDaemonScript(): string | null {
		try {
			const scriptPath = this.getDaemonScriptPath();
			if (!existsSync(scriptPath)) {
				return null;
			}
			return createHash("sha1").update(readFileSync(scriptPath)).digest("hex");
		} catch {
			return null;
		}
	}
	...
	private isDaemonScriptStale(): boolean {
		try {
			if (!existsSync(SCRIPT_HASH_PATH)) {
				return false; // No hash file = first run or manual cleanup
			}

			const savedHash = readFileSync(SCRIPT_HASH_PATH, "utf-8").trim();
			const currentHash = this.hashDaemonScript();
			if (!currentHash) {
				return false;
			}
	...
	/**
	 * Save the daemon bundle's hash to detect rebuilds.
	 */
	private saveDaemonScriptHash(): void {
		try {
			const hash = this.hashDaemonScript();
			if (!hash) {
				return;
			}
			writeFileSync(SCRIPT_HASH_PATH, hash, { mode: 0o600 });
		} catch {
			// Best-effort
		}
	}
	...
	private acquireSpawnLock(): boolean {
		try {
			// Ensure odin home directory exists before any file operations
			if (!existsSync(ODIN_HOME_DIR)) {
				mkdirSync(ODIN_HOME_DIR, { recursive: true, mode: 0o700 });
			}
			try {
				chmodSync(ODIN_HOME_DIR, 0o700);
			} catch {
				// Best-effort.
			}
	...
	private async spawnDaemon(): Promise<void> {
		...
		try {
			...
			const logPath = join(ODIN_HOME_DIR, "daemon.log");
			let logFd: number;
			try {
			...
			// Detached in dev too, so sessions survive an electron-vite restart
			// (and `bun dev` exiting). A daemon running stale terminal-host code is
			// caught at boot by the bundle-hash check, not by killing it every time.
			let child: ReturnType<typeof spawn> | null = null;
			try {
				child = spawn(electronPath, [daemonScript], {
					detached: true,
			...
			// spawn() reports ENOENT asynchronously; unhandled, it lands as an
			// uncaught exception in the main process and the real cause is lost
			// behind the wait timeout below.
			...
			await this.waitForDaemon(() => spawnError);

			// In development mode, save the bundle hash to detect rebuilds
			if (process.env.NODE_ENV === "development") {
				this.saveDaemonScriptHash();
			}
	...
	private async waitForDaemon(
		...
		while (Date.now() - startTime < SPAWN_WAIT_MS) {
			...
			if (spawnError) {
				throw new Error(`Daemon failed to spawn: ${spawnError.message}`);
			}
			if (existsSync(SOCKET_PATH)) {
