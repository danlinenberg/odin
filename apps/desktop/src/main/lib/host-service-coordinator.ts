...
import { organizations, settings } from "@odin/local-db";
import { getHostId, getHostName } from "@odin/shared/host-info";
...
export class HostServiceCoordinator extends EventEmitter {
	...
	async start(organizationId: string): Promise<Connection> {
		assertSafeOrganizationId(organizationId);
		return this.startWithPreferredPorts(organizationId);
	}
	...
	private async startWithPreferredPorts(
		...
		if (pending) {
			...
			return this.startWithPreferredPorts(organizationId, preferredPorts);
		}
	...
	async restart(organizationId: string): Promise<Connection> {
		assertSafeOrganizationId(organizationId);
	...
	async restartAll(): Promise<void> {
		await Promise.all(
			this.getActiveOrganizationIds().map((orgId) => this.restart(orgId)),
		);
	}
	 ...
	 */
	async reconcile(organizationIds: Iterable<string>): Promise<void> {
		this.startGeneration++;
		...
		await Promise.all(
			[...this.desiredOrganizationIds].map(async (organizationId) => {
				try {
					await this.start(organizationId);
				} catch (error) {
	 ...
	 * instance sharing this machine's `$ODIN_HOME_DIR`.
	 *
	...
	private async spawn(
		...
		const childEnv = await this.buildEnv(organizationId, port, secret);
		if (!isStartAllowed()) {
	...
	private async buildEnv(
		...
		const childEnv = await getProcessEnvWithShellPath({
			...
			ODIN_AUTH_CONFIG_PATH: path.join(ODIN_HOME_DIR, "config.json"),
		...
		// The host service is reachable on loopback only. A RELAY_URL inherited
		// from the user's interactive shell must not reach the child.
	...
	private async respawn(
		...
		// across an await, and a stop() or stopAll() in that gap must abandon the
		// attempt: otherwise a fresh child is spawned and registered after
