...
export function resolvePaneIdFromTabsState(
	...
	// A hook that lost its pane env still carries the agent's conversation id.
	// Chat panes keep it under `chat`, terminal sessions under `claudeSessionId`.
	if (sessionId) {
		for (const [existingPaneId, pane] of Object.entries(
			tabsState.panes ?? {},
		)) {
			if (
				pane.chat?.sessionId === sessionId ||
