...
describe("extractWorkspaceIdFromUrl", () => {
	...
	it("extracts workspace ID when file path contains /workspace/", () => {
		...
		const url =
			"file:///Users/foo/workspace/odin/dist/index.html#/workspace/def456";
...
describe("getNotificationTitle", () => {
	...
	it("returns null when the event belongs to no known pane", () => {
		expect(getNotificationTitle({})).toBeNull();
	});

	it("strips the agent's spinner glyph", () => {
		const spinning = [{ id: "t1", name: "\u2733 Claude Code" }];
		expect(getNotificationTitle({ tabId: "t1", tabs: spinning })).toBe(
			"Claude Code",
		);
	});
