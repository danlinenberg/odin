...
describe("NotificationManager", () => {
	...
	describe("tracking and replacement", () => {
		...
		it("replaces a pane-less notification when the same session later resolves a pane", () => {
			...
			manager.handleAgentLifecycle(
				makeEvent({ eventType: "Start", sessionId: "session-1" }),
			);
			manager.handleAgentLifecycle(
				makeEvent({
					eventType: "Stop",
	...
	describe("needs-you gate", () => {
		it("stays silent when a parked card just ends its turn", () => {
			const deps = createDeps({ isParked: () => true });
		...
		// Parking pins an idle session to Idle; it does not mute a prompt that
		// blocks on you. boardColumn keeps that card in Needs you, so the banner
		// follows it there.
		it("still notifies when a parked card raises a prompt", () => {
			const deps = createDeps({ isParked: () => true });
			const manager = new NotificationManager(deps);
		...
		it("notifies for an unparked session that ended its turn", () => {
		...
		it("notifies on the move into Needs you, not on every turn after", () => {
			const deps = createDeps();
			const manager = new NotificationManager(deps);
		...
		it("notifies again once the agent has gone back to working", () => {
			const deps = createDeps();
			const manager = new NotificationManager(deps);
			...
			expect(deps.notifications).toHaveLength(2);
		});
	});

	describe("notification content", () => {
		it("names the task and why it needs you for PermissionRequest events", () => {
			...
			expect(createNotification).toHaveBeenCalledWith(
				expect.objectContaining({
					title: "Test Title needs you",
					body: "Waiting for your reply",
				}),
			);
		});

		it("names the task and why it needs you for Stop events", () => {
			const createNotification = mock(
				(_opts: { title: string; body: string; silent: boolean }) =>
					createMockNotification(),
			);
		...
		it("stays quiet for an agent running outside any known pane", () => {
			const localDeps = createDeps({ getNotificationTitle: () => null });
			const localManager = new NotificationManager(localDeps);

			localManager.handleAgentLifecycle(makeEvent({ eventType: "Stop" }));

			expect(localDeps.notifications).toHaveLength(0);
		});
	});
});
