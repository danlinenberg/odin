...
describe("runQuitCleanup", () => {
	...
	// A dev quit is usually electron-vite restarting the app after an edit. Tearing
	// the daemon down there killed every PTY, so each restart meant resuming every
	// session by hand.
	test("leaves the terminal host running on a dev quit", async () => {
		const h = createHarness({ isDev: true });
