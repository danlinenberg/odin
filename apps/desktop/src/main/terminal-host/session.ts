...
import {
	...
	scanForShellReady,
} from "@odin/shared/shell-ready-scanner";
...
import { resolveElectronBinary } from "../lib/terminal-host/electron-binary";
...
/** True if `promise` resolved within `timeoutMs`; false if it stalled or threw. */
function resolvedWithin(
	...
	return Promise.race([
		promise.then(
			() => true,
			// A late rejection would otherwise be unhandled once we've timed out.
			() => false,
		),
...
export class Session {
	...
	spawn(options: {
		...
		// Resolved rather than taken from process.execPath: the daemon outlives a
		// dev bundle rename, and the path it launched from stops existing.
	...
	async attach(
		...
		try {
			...
			// The emulator's own parse queue is driven by xterm's async write
			...
			const flushed = await raceWithAbort(
				resolvedWithin(this.emulator.flush(), ATTACH_FLUSH_TIMEOUT_MS),
				signal,
			);
			if (!flushed) {
				console.warn(
					`[Session ${this.sessionId}] Emulator flush timeout after ${ATTACH_FLUSH_TIMEOUT_MS}ms`,
				);
			}
			throwIfAborted(signal);
