...
describe("Terminal Host Session shell args", () => {
	...
	it("returns a snapshot even when the emulator flush never settles", async () => {
		const session = new Session({
			sessionId: "session-stalled-flush",
			workspaceId: "workspace-1",
			paneId: "pane-1",
			tabId: "tab-1",
			cols: 80,
		...
		// A stalled xterm write pump used to hang attach forever: the client gave
		// up after 30s and the pane never started streaming (blank session).
		const emulator = (
