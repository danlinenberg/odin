...
/** Size for a PTY we spawn before any view has measured itself. */
			...
			if (!session) {
				...
				try {
					...
					session.spawn({
						...
						cols: request.cols ?? DEFAULT_SPAWN_COLS,
						rows: request.rows ?? DEFAULT_SPAWN_ROWS,
						env: request.env,
					});
				...
				isNew = true;
			} else if (request.cols !== undefined && request.rows !== undefined) {
