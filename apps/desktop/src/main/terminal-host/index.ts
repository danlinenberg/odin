 ...
 * - Socket: ~/.odin/terminal-host.sock
 * - Auth token: ~/.odin/terminal-host.token
 */
...
// ODIN_DIR_NAME is imported from shared/constants for multi-worktree support
// This allows workspace-specific home directories (e.g., ~/.odin-my-feature)
...
async function startServer(): Promise<void> {
	// Ensure odin directory exists with proper permissions
	if (!existsSync(ODIN_HOME_DIR)) {
		mkdirSync(ODIN_HOME_DIR, { recursive: true, mode: 0o700 });
		log("info", `Created directory: ${ODIN_HOME_DIR}`);
	}
...
async function main() {
	...
	log("info", `Home directory: ${ODIN_HOME_DIR}`);
