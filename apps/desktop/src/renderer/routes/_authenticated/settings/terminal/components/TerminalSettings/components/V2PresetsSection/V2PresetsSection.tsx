...
import { HOST_AGENT_PRESETS } from "@odin/shared/host-agent-presets";
...
export function V2PresetsSection({
	...
	const { data: v2Presets = [] } = useLiveQuery(
		(query) =>
			query
				.from({ presets: v2TerminalPresets })
				.orderBy(({ presets }) => presets.tabOrder),
		[],
	);
	...
	const updateV2Preset = useCallback(
		(id: string, patch: Partial<V2TerminalPresetRow>) => {
			v2TerminalPresets.update(id, (draft) => {
	...
	const deleteV2Preset = useCallback((id: string) => {
		v2TerminalPresets.delete(id);
	}, []);
