...
} from "@odin/shared/host-agent-presets";
import { Skeleton } from "@odin/ui/skeleton";
...
import { v2TerminalPresets } from "renderer/lib/local-collections";
...
function insertLinkedTerminalPreset(agent: HostAgentConfig): void {
	if (agent.command.trim().length === 0) return;
	const presets = [...v2TerminalPresets.values()];
...
export function V2AgentsSettings({
	...
	const syncLinkedPresetSnapshots = (updated: HostAgentConfig) => {
		...
		for (const preset of v2TerminalPresets.values()) {
			if (
			...
			v2TerminalPresets.update(preset.id, (draft) => {
	...
	const addMutation = useMutation({
		...
		onSuccess: (added) => {
			...
			if (added?.id) {
				...
				insertLinkedTerminalPreset(added);
			}
		},
		onError: (err) =>
