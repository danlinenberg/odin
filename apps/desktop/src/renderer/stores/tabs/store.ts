				...
				merge: (persistedState, currentState) => {
					...
					if (persisted.panes) {
						for (const pane of Object.values(persisted.panes)) {
							...
							// them still are. Clearing it made every working card flash
							// through Needs you (alive + idle = "wants a look") before the
							// screen scan put it back. A pane whose session really did die
							// is demoted by the daemon poll (boardColumn: alive === false →
							// Idle) and by the terminal's own exit handler.
							// Odin fork: on relaunch the daemon is gone and every PTY is
							// dead. Flag every restored terminal pane (except ones the user
							// killed) as interrupted so its card shows a Resume affordance.
							// Killed panes carry `completed` and stay in the hidden
							// Completed section; a clean restart hides nothing you didn't
							// dismiss.
