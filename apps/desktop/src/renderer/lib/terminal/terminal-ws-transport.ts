import {
	createHostSocket,
	type HostSocket,
} from "@odin/workspace-client/host-socket";
...
export interface TerminalTransport {
	...
	/** Internal: the shared reconnecting socket (partysocket). Created once on
	 * first connect; it re-signs the URL before every (re)dial and retries
	 * indefinitely. */
	_socket: HostSocket | null;
	...
	/** Token carried on the URL the caller passed, reused as-is per dial. */
...
function maybeSurfaceDiagnosis(
	...
	const diagnosis = TERMINAL_CONNECTION_LOST;
...
// The URL minus its token param. createHostSocket signs a fresh token onto it
	...
	socket.addEventListener("error", () => {
		...
		if (
			...
			pushLog(
				transport,
				"error",
				`WebSocket error while connecting to ${formatWsEndpoint(transport.currentUrl)}. Check that the host service is running.`,
			);
		}
