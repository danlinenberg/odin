...
async function writeTaskPromptFile(
	...
	const briefDirectory = joinAbsolutePath(workspace.worktreePath, BRIEF_DIR);
	await electronTrpcClient.filesystem.createDirectory.mutate({
		workspaceId,
		absolutePath: briefDirectory,
		recursive: true,
	});
	await electronTrpcClient.filesystem.writeFile.mutate({
		workspaceId,
		absolutePath: joinAbsolutePath(briefDirectory, baseName),
		content,
		encoding: "utf-8",
	});
}
...
async function writeAttachmentFiles(
	...
	for (let i = 0; i < parsedFiles.length; i++) {
		...
		writtenPaths.push(`${BRIEF_DIR}/attachments/${fileName}`);
	}
