...
// Locks in the shape of shipped defaults so layout mapping keeps doing
// something. A bare-string (physical) chord short-circuits in
// bindingToDispatchChord, so defaults written that way ignore the OS layout
// entirely. Keeping printable defaults as `mode: "logical"` is what makes
// them follow the key labels on non-US layouts.
