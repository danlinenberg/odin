...
export function normalizeWorkspaceName(name?: string): string | undefined {
	if (!name || name === "odin") return undefined;
	return name
		.toLowerCase()
		.replace(/[^a-z0-9-]/g, "-")
		.slice(0, 32);
 ...
 * ~/.odin/worktrees/<project>/...
 *
 * Examples:
 * - ["odin", "my-branch", "apps", "desktop"] -> "my-branch"
 * - ["odin", "owner", "workspace", "apps", "desktop"] -> "owner-workspace"
 * - ["odin", "owner", "feature", "x", "apps", "desktop"] -> "owner-feature-x"
 */
...
export function getWorkspaceName(): string | undefined {
	return normalizeWorkspaceName(process.env.ODIN_WORKSPACE_NAME);
}
