...
const envSchema = z.object({
	...
	ODIN_WORKSPACE_NAME: z.string().default("odin"),
});
...
export const env = envSchema.parse({
	...
	ODIN_WORKSPACE_NAME: process.env.ODIN_WORKSPACE_NAME,
});

export function getWorkspaceName(): string | undefined {
	const name = env.ODIN_WORKSPACE_NAME;
	if (name === "odin") return undefined;
	return name
		.toLowerCase()
		.replace(/[^a-z0-9-]/g, "-")
		.slice(0, 32);
