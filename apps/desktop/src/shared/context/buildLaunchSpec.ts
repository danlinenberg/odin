import { renderPromptTemplate } from "@odin/shared/agent-prompt-template";
import type { ResolvedAgentConfig } from "@odin/shared/agent-settings";
...
function renderAttachmentsList(sections: ContextSection[]): string {
	...
	for (const section of sectionsOfKind(sections, "attachment")) {
		refs.push(`- ${BRIEF_DIR}/attachments/${section.label}`);
	}
	for (const section of sectionsOfKind(sections, "user-prompt")) {
		for (const part of section.content) {
			...
			refs.push(`- ${BRIEF_DIR}/attachments/${label ?? "inline-attachment"}`);
		}
	}
