...
import { projects, type SelectProject } from "@odin/local-db";
...
function getConfigPath(mainRepoPath: string): string {
	return join(mainRepoPath, ".odin", "config.json");
}

function ensureConfigExists(mainRepoPath: string): string {
	const configPath = getConfigPath(mainRepoPath);
	const odinDir = join(mainRepoPath, ".odin");

	if (!existsSync(configPath)) {
		// Create .odin directory if it doesn't exist
		if (!existsSync(odinDir)) {
			mkdirSync(odinDir, { recursive: true });
		}
