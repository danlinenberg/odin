...
const TEST_DIR = join(realpathSync(tmpdir()), `odin-test-git-${process.pid}`);
...
describe("getDefaultBranch", () => {
	...
	function createIsolatedTestRepo(testName: string): {
		...
		const testDir = join(
			realpathSync(tmpdir()),
			`odin-test-${testName}-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
...
describe("Shell Environment", () => {
	...
	test("getProcessEnvWithShellPath applies shell PATH and preserves string vars", async () => {
		...
		if (shellPath) {
			// On macOS the result is the shell PATH with any missing homebrew/common
			// dirs prepended (augmentPathForMacOS), so assert suffix, not equality.
	...
	test("getShellEnvironment captures .zshrc variables (requires -ilc)", async () => {
		...
		writeFileSync(
			join(tmpDir, ".zshrc"),
			'export __ODIN_SHELL_ENV_TEST__="interactive"\n',
		);
		...
		try {
			const env = await getShellEnvironment();
			expect(env.__ODIN_SHELL_ENV_TEST__).toBe("interactive");
		} finally {
...
describe("getCurrentBranch", () => {
	test("returns branch name for empty repo with unborn HEAD", async () => {
		const repoPath = join(
			realpathSync(tmpdir()),
			`odin-test-current-branch-empty-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
	...
	test("returns null in detached HEAD state", async () => {
		const repoPath = join(
			realpathSync(tmpdir()),
			`odin-test-current-branch-detached-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
...
describe("parsePrUrl", () => {
	test("parses canonical GitHub PR URL", () => {
		expect(
			parsePrUrl("https://github.com/danlinenberg/odin/pull/1781"),
		).toEqual({
			owner: "danlinenberg",
			repo: "odin",
			number: 1781,
	...
	test("parses GitHub URL without protocol", () => {
		expect(parsePrUrl("github.com/danlinenberg/odin/pull/1781")).toEqual({
			owner: "danlinenberg",
			repo: "odin",
			number: 1781,
		});
	...
	test("returns null for non-PR URLs", () => {
		expect(parsePrUrl("https://github.com/danlinenberg/odin/issues/1781")).toBe(
			null,
		);
	});
});
