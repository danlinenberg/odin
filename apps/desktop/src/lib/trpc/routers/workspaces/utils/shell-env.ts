...
// In-flight refresh, so concurrent callers share one shell spawn instead of
// racing several: a single session launch fires several git calls at once.
 ...
 *
 * Results are cached for 1 minute, and a stale cache is served while it
 * refreshes in the background, so only the first read of the app's life waits
 * on a shell.
 */
export async function getShellEnvironment(
	options?: GetShellEnvironmentOptions,
): Promise<Record<string, string>> {
	if (options?.forceRefresh) return refreshShellEnvironment();
	...
	if (cachedEnv) {
		...
		if (Date.now() - cacheTime >= ttl) {
			void refreshShellEnvironment().catch(() => {});
		}
		return { ...cachedEnv };
	}

	return refreshShellEnvironment();
}

/** Spawns the login shell and replaces the cache. At most one runs at a time. */
function refreshShellEnvironment(): Promise<Record<string, string>> {
	refreshInFlight ??= (async () => {
		try {
			const env = (await getShellEnvWithTimeout()) as Record<string, string>;
			cachedEnv = env;
			cacheTime = Date.now();
