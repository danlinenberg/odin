import type { SelectWorktree } from "@odin/local-db";
import { projects, workspaces, worktrees } from "@odin/local-db";
...
import { copyOdinConfigToWorktree, loadSetupConfig } from "./setup";
 ...
 * 4. Imports the worktree into the database with createdByOdin=false
...
> {
	...
	try {
		...
		const worktree = existingWorktreeByPath
			...
			: localDb
					...
					.values({
						...
						createdByOdin: false, // Mark as external
					})
					.returning()
					.get();
		...
		copyOdinConfigToWorktree(project.mainRepoPath, externalMatch.path);
...
export async function openExternalWorktree({
	...
	if (existingWorktree) {
		...
		copyOdinConfigToWorktree(project.mainRepoPath, existingWorktree.path);
		const setupConfig = loadSetupConfig({
			mainRepoPath: project.mainRepoPath,
			worktreePath: existingWorktree.path,
			projectId: project.id,
	...
	const worktree = localDb
		...
		.values({
			...
			createdByOdin: false, // External worktree
		})
		.returning()
		.get();
	...
	copyOdinConfigToWorktree(project.mainRepoPath, worktreePath);
	const setupConfig = loadSetupConfig({
		mainRepoPath: project.mainRepoPath,
		worktreePath,
		projectId: project.id,
