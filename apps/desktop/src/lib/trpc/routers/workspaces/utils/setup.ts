...
export function copyOdinConfigToWorktree(
	...
	const mainOdinDir = join(mainRepoPath, PROJECT_ODIN_DIR_NAME);
	const worktreeOdinDir = join(worktreePath, PROJECT_ODIN_DIR_NAME);

	if (existsSync(mainOdinDir) && !existsSync(worktreeOdinDir)) {
		try {
			cpSync(mainOdinDir, worktreeOdinDir, { recursive: true });
		} catch (error) {
			console.error(
				`Failed to copy ${PROJECT_ODIN_DIR_NAME} to worktree: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}
}
...
function readConfigFromPath(basePath: string): SetupConfig | null {
	return readConfigFile(
		join(basePath, PROJECT_ODIN_DIR_NAME, CONFIG_FILE_NAME),
	);
}
...
function readLocalConfigFromPath(basePath: string): LocalSetupConfig | null {
	return readLocalConfigFile(
		join(basePath, PROJECT_ODIN_DIR_NAME, LOCAL_CONFIG_FILE_NAME),
	);
}
 ...
 *   1. User override:  ~/.odin/projects/<projectId>/config.json
 *   2. Worktree:       <worktreePath>/.odin/config.json
 *   3. Main repo:      <mainRepoPath>/.odin/config.json
 *
 ...
 * `.odin/config.local.json` exists in the workspace (worktree or main repo).
