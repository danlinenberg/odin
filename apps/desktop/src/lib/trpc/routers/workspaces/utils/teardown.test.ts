...
import { ODIN_DIR_NAME, PROJECTS_DIR_NAME } from "shared/constants";

const TEST_DIR = join(tmpdir(), `odin-test-teardown-${process.pid}`);
const TEST_ODIN_HOME = join(TEST_DIR, "odin-home");
...
const ORIGINAL_ODIN_HOME_DIR = process.env.ODIN_HOME_DIR;
...
describe("runTeardown", () => {
	beforeEach(() => {
		process.env.ODIN_HOME_DIR = TEST_ODIN_HOME;
		// Create test directories
		mkdirSync(join(MAIN_REPO, ".odin"), { recursive: true });
	...
	afterEach(() => {
		...
		if (ORIGINAL_ODIN_HOME_DIR === undefined) {
			delete process.env.ODIN_HOME_DIR;
		} else {
			process.env.ODIN_HOME_DIR = ORIGINAL_ODIN_HOME_DIR;
		}
	...
	test("returns success when config has no teardown commands", async () => {
		writeFileSync(
			join(MAIN_REPO, ".odin", "config.json"),
	...
	test("uses worktreePath config when present", async () => {
		...
		mkdirSync(join(WORKTREE, ".odin"), { recursive: true });
		writeFileSync(
			join(WORKTREE, ".odin", "config.json"),
	...
	test("sets environment variables for teardown scripts", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				teardown: [
					`echo "$ODIN_WORKSPACE_NAME|$ODIN_ROOT_PATH" > "${envFile}"`,
				],
			}),
		);
	...
	test("uses managed wrapper path for teardown managed binaries even with PATH rewrites", async () => {
		...
		const wrapperBinDir = join(TEST_ODIN_HOME, "bin");
