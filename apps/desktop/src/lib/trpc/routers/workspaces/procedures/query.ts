...
export const createQueryProcedures = () => {
	return router({
				...
				return {
					...
					worktree: worktree
						? {
								...
								createdByOdin: worktree.createdByOdin,
							}
						: null,
				};
			}),
		...
		getAllGrouped: publicProcedure.query(() => {
			...
			const worktreeCreatedByOdinMap = new Map(
				allWorktrees.map((wt) => [wt.id, wt.createdByOdin]),
			);
			...
			for (const workspace of allWorkspaces) {
				...
				if (group) {
					...
					const item: WorkspaceItem = {
						...
						createdByOdin: workspace.worktreeId
							? (worktreeCreatedByOdinMap.get(workspace.worktreeId) ?? null)
							: null,
					};
