 ...
 * 2. The createdByOdin flag is correctly set
 * 3. External worktrees are not deleted from disk when workspace is removed
 */

const TEST_DIR = join(
	realpathSync(tmpdir()),
	`odin-test-external-wt-${process.pid}`,
);
...
describe("External worktree detection and import", () => {
	...
	test("external worktree can be created and detected", () => {
		// Create external worktree manually (simulates user creating it outside Odin)
	...
	test("external worktree data survives simulated deletion", () => {
		...
		// the createdByOdin flag and skip disk deletion for external worktrees
